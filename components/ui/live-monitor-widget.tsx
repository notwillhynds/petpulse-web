...
type Metric = {
  key: 'breathRate' | 'heartRate' | 'steps' | 'temp' | 'sleep';
  label: string;
  unit: string;
  colorClass: string;
...
function nextValue(key: Metric['key'], prev: number) {
  switch (key) {
    case 'breathRate':
      return Math.round(clamp(prev + jitter(6), 10, 30));
    case 'heartRate':
      return Math.round(clamp(prev + jitter(5), 68, 160));
    case 'steps':
      return Math.round(clamp(prev + jitter(180), 120, 4200));
    case 'temp':
...
export function LiveMonitorWidget() {
  const [series, setSeries] = useState<MetricSeries>({
    breathRate: Array.from({ length: SERIES_POINTS }, (_, i) => 12 + i),
    heartRate: Array.from({ length: SERIES_POINTS }, (_, i) => 96 + i),
  ...
  return (
    <div className="border-border/70 bg-card/70 rounded-2xl border p-4 shadow-sm backdrop-blur">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-foreground text-sm font-semibold">Live Monitor</p>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {metrics.map((metric) => {
          ...
          return (
            <div
              key={metric.key}
              className="border-border/60 bg-background/80 rounded-xl border p-2.5"
            >
              <div className="mb-1.5 flex items-center justify-between gap-2">
                <div className="inline-flex min-w-0 items-center gap-1.5">
                  <Icon className={`h-3.5 w-3.5 shrink-0 ${metric.colorClass}`} />
